...
describe('hairstyle catalog', () => {
  it('maps every tribe and gender to the matching CharaMakeType row', () => {
    expect(charaMakeTypeRow(1, 'male')).toBe(0)
    expect(charaMakeTypeRow(1, 'female')).toBe(1)
    expect(charaMakeTypeRow(16, 'male')).toBe(30)
    expect(charaMakeTypeRow(16, 'female')).toBe(31)
  })

  it('requests only the hairstyle menu fields', () => {
    const url = hairstyleMenuUrl(7, 'male')
    expect(url.pathname).toBe('/api/sheet/CharaMakeType/12')
