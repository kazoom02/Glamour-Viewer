...
The preview uses the authored LOD0 mesh, trilinear mip filtering, the browser GPU's maximum anisotropy, sRGB output, and neutral tone mapping. Weapon IMC material-animation IDs resolve the matching `material.pap`; its Havok tracks are sampled against `skl_material.sklb`, and the authored translation RGB curves drive emitting materials. For equipment/weapon VFX IDs, the browser reads the local `.avfx` and interprets its scheduler/timeline links, emitter-to-particle graph, Const/Repeat/Add curves, linear/step/spline keys, UV animation, animated `TLst` texture sequences, texture-palette lookups, blend/depth settings, indexed ATEX bindings, and embedded draw/emitter meshes. The runtime is still a browser reimplementation rather than Square Enix's native renderer: effectors, screen-copy/distortion passes, lights, sound, and every specialized particle shader are not yet exact.

In local mode the viewer also exposes an **animation catalog**. `public/animation-catalog.json` lists the human-character animation reference names (idles, emotes, `/pose` stances, dances, movement loops, crafting/gathering, and combat clips); each maps to a `chara/human/{race}/animation/a0001/{weaponClass}/{sub}/{papFile}.pap` path. Selecting one decodes that PAP from the user's own install — the same Havok spline decoder used for the default idle — selects the requested track, and plays it on the live rig, keeping the character centred and any female bust scale applied. The catalog is metadata only; no Square Enix animation bytes are shipped. Additive overlay clips (most combat abilities/weaponskills) are reported rather than played, since they blend over a base pose rather than standing alone.
