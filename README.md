...
The MTRL dye-row templates/channels are parsed, but applying a user-selected dye still requires the game stain-template (`.stm`) data and a dye picker. PAP animation sampling, PBD/EQDP deformation, and detailed equipment body-hiding metadata are also still required for animated, dye-accurate Fanbyte-level parity. Until PBD/EQDP deformation is implemented, a missing race-specific equipment model uses the closest available body-family fallback and may not match the in-game silhouette perfectly.
