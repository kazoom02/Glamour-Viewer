...
The current viewer decodes the actual base body pieces, face, hair, and equipped armor from `040000.win32.index2`/`datN` in a module worker. It assembles them in their shared game bind pose, replaces the covered `e0000` body slot when armor is equipped, and exposes drag/zoom inspection. Midlander male (`c0101`), Midlander female (`c0201`), and Roegadyn male (`c0901`) have complete standalone base parts and are selectable. The decoder also extracts material references, bone names, palettes, blend indices, and normalized weights.

Rendering currently uses representative solid materials. SKLB/Havok pose decoding, MTRL/TEX texture rendering, IMC mesh masks, and PBD/EQDP deformation are still required for animated, textured Fanbyte-level parity. Other playable races remain hidden from the selector until their shared-body deformation rules are implemented rather than being shown with an incorrect Midlander body.
