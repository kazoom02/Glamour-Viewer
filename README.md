...
The current viewer decodes the actual base body pieces, face, hair, and equipped armor from `040000.win32.index2`/`datN` in module workers. It assembles them in their shared game bind pose, replaces the covered `e0000` body slot when armor is equipped, and exposes drag/zoom inspection. Midlander male (`c0101`), Midlander female (`c0201`), and Roegadyn male (`c0901`) have complete standalone base parts and are selectable.

For local installs, the browser also reads the equipment IMC row selected by XIVAPI's model variant, resolves the corresponding `material/v####` MTRL files, follows their shader sampler references, and decodes TEX surfaces including BC1, BC3, BC5, and BC7. Diffuse, reconstructed tangent-space normal, and mask/roughness textures are applied with Three.js. Decoded RGBA surfaces are cached in the material worker and in browser IndexedDB; no cache entry or game byte leaves the device.

SKLB/Havok pose decoding, MTRL color-set/dye shaders, IMC mesh visibility masks, and PBD/EQDP deformation are still required for animated, dye-accurate Fanbyte-level parity. Other playable races remain hidden from the selector until their shared-body deformation rules are implemented rather than being shown with an incorrect Midlander body.
